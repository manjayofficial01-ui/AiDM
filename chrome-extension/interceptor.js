...
(function () {
  ...
  function notifyUrl(url, extra) {
    ...
    try { if (typeof isSegmentUrl === 'function' && isSegmentUrl(url)) return; } catch (e) {}
    try { if (typeof isTwitterPlaylistUrl === 'function' && isTwitterPlaylistUrl(url)) return; } catch (e) {}
    // Twitter/X MP4s carry a rotating ?tag=… — strip the query so the same
    // variant is not notified (and offered) once per poll.
    try {
      if (/video\.twimg\.com/i.test(url) && /\/vid\/|ext_tw_video|amplify_video/i.test(url)) {
        url = url.split('#')[0].split('?')[0];
        if (extra && extra.url) delete extra.url;
      }
    } catch (e) {}
  ...
  const CDN_RE = /googlevideo\.com|cdn.*video|video.*cdn|\.cdn\.|akamaihd\.net|cloudfront\.net|fastly|fbcdn\.net|scontent\.|cdninstagram\.com/i;
  const XTREAM_RE = /\/(live|movie|series)\/[^/?#]+\/[^/?#]+\/\d+(?:\.(m3u8|ts|mp4|mkv|avi))?(?:$|[/?#])/i;
  const PATH_RE = /videoplayback|\/get_file\/|\/hls\/|\/dash\/|\/mp4\/|\/videos?\/|video\.php|watchparty|\/reel/i;
  const IMAGE_RE = /\.(jpe?g|png|gif|webp|avif|bmp|svg|tiff?|ico|heic|heif)(\?|#|$)/i;
  // Facebook video hosts — extensionless …/v/t… URLs carry no .mp4 suffix.
  const FB_HOST_RE = /fbcdn\.net|scontent\.|facebook\.com|fb\.com|instagram\.com|cdninstagram\.com/i;
  const FB_VIDEO_PATH_RE = /\/v\/t|video\.php|playable|bytestart|\/dash\/|\/hls\//i;
  // Raw DASH/HLS segments are never playable downloads — one video would
  // otherwise surface as dozens of seg-3.m4s / chunk rows.
  const SEGMENT_RE = /\.m4s($|\?|#|;)|init\.mp4($|\?|#)|seg-?\d+|chunklist|fragment|frag-?\d+|\/range\//i;

  function isSegmentUrl(url) {
    if (!url || typeof url !== 'string') return false;
    if (/\.m4s($|\?|#|;)/i.test(url)) return true;
    // Facebook ?bytestart=N range-slices are partial chunks of the progressive
    // MP4, not standalone files — but unlike .m4s they ARE recoverable: the
    // caller rewrites them to the full-file URL via stripFbRange() BEFORE
    // dedup/download. So they must NOT be dropped outright here.
    if (/bytestart=\d+/i.test(url) && /fbcdn\.net|scontent\.|facebook\.com|cdninstagram\.com/i.test(url)) return false;
    ...
    if (/\.ts($|\?|#|;)/i.test(url) && /twimg\.com/i.test(url)) return true;
    if (/\.ts($|\?|#|;)/i.test(url) && /seg|chunk|frag|part|range|hls|dash|playlist|media|sq_|index|seq/i.test(url)) return true;
  ...
  function looksLikeMedia(url) {
    ...
    if (isTwitterPlaylistUrl(url)) return false;
    // Images are NEVER downloadable video — reject before any broad CDN/path
    // heuristic can match. Facebook alone loads dozens of scontent images.
    if (IMAGE_RE.test(url)) return false;
    if (VIDEO_RE.test(url) || STREAM_RE.test(url) || AUDIO_RE.test(url)) return true;
    // Facebook / Instagram video: require a video-specific host AND path.
    // A bare fbcdn.net + /v/ match is too broad — images also use /v/t1/.
    if (FB_HOST_RE.test(url) && FB_VIDEO_PATH_RE.test(url)) return true;
    if (FB_HOST_RE.test(url) && /\/v\/t4[0-9]\//i.test(url)) return true; // FB video quality buckets t40-t49
    if (XTREAM_RE.test(url)) return true;
    // Generic CDN / path heuristics — but only when NOT on an image CDN path
    if (CDN_RE.test(url) && !/fbcdn|scontent|cdninstagram/i.test(url)) return true;
    if (PATH_RE.test(url) && !IMAGE_RE.test(url)) return true;
    return false;
  }
  ...
  function looksLikeMediaContentType(ct) {
    ...
    ct = ct.toLowerCase();
    // image/* is NOT downloadable video — Facebook serves hundreds of images
  ...
  //
  // The player also polls …/pu/pl/…m3u8?tag=… continuously while playing. Those
  // playlist URLs are encrypted and must never be offered — one playing video
  // would otherwise surface as 100+ identical capsule rows.

  function isTwitterPlaylistUrl(url) {
    if (!url || typeof url !== 'string') return false;
    if (!/video\.twimg\.com|pbs\.twimg\.com|t\.twimg\.com/i.test(url)) return false;
    return /\/pl\//i.test(url) || /\.(m3u8|mpd)(\?|#|$|;)/i.test(url);
  }
  ...
  function notifyTwitterMp4s(text, via) {
    try {
      // Attributed first: walk the parsed JSON so each variant set stays glued
      // to its own tweet id. The capsule uses that to show THIS video's rows
      // instead of the whole timeline's (the 131-identical-rows bug).
      let attributed = 0;
      try {
        const groups = extractTwitterVariantsByTweet(text);
        for (const g of groups) {
          if (!g || !g.tweetId || !g.urls || !g.urls.length) continue;
          attributed += g.urls.length;
          postToContentScript('twitter-variants', { tweetId: String(g.tweetId), urls: g.urls, via });
          g.urls.forEach(u => notifyUrl(u, { via, contentType: 'video/mp4' }));
        }
      } catch (e) { /* fall through to unattributed */ }
      if (!attributed) {
  ...
  /**
   * Parse a GraphQL/timeline JSON body into per-tweet variant groups:
   *   [{ tweetId, urls: [mp4…] }]
   * A Tweet node carries its id in `rest_id` (or legacy.id_str) with the
   * media's video_info.variants nested somewhere underneath, so a recursive
   * walk that inherits the nearest tweet id attributes every variant
   * correctly — even for timelines that batch dozens of tweets per response.
   * Returns [] when the body is not JSON or holds no attributable variants.
   */
  function extractTwitterVariantsByTweet(text) {
    if (!maybeTwitterVideoJson(text)) return [];
    let root = null;
    try { root = JSON.parse(text); }
    catch (e) { return []; }
    const groups = new Map(); // tweetId -> Set(url)
    const isTweetId = (s) => typeof s === 'string' && /^\d{5,25}$/.test(s);
    const cleanMp4 = (u) => {
      if (!u || typeof u !== 'string' || !/^https?:/i.test(u)) return null;
      if (!/\.mp4(\?|#|$)/i.test(u)) return null;
      if (!/twimg\.com|ext_tw_video|amplify_video/i.test(u)) return null;
      try {
        const noFrag = decodeJsonUrl(u).split('#')[0];
        const cut = Math.min(
          noFrag.indexOf('?') >= 0 ? noFrag.indexOf('?') : Infinity,
          noFrag.indexOf('&') >= 0 ? noFrag.indexOf('&') : Infinity
        );
    ...
    const variantsOf = (node) => {
      const out = [];
      try {
        const vi = node && node.video_info && node.video_info.variants;
        if (Array.isArray(vi)) {
          for (const v of vi) {
            const u = cleanMp4(v && v.url);
            if (u && !isSegmentUrl(u)) out.push(u);
          }
        }
      } catch (e) {}
      return out;
    };
    const walk = (node, tweetId, depth) => {
      if (!node || depth > 60) return;
      if (Array.isArray(node)) {
        for (const el of node) walk(el, tweetId, depth + 1);
        return;
      }
      if (typeof node !== 'object') return;
      let cur = tweetId;
      try {
        // A result node with its own rest_id starts a new tweet scope —
        // but media objects also carry id_str (media id), which must NOT
        // override the enclosing tweet scope. rest_id is tweet-scoped.
        if (isTweetId(node.rest_id)) cur = node.rest_id;
        else if (node.legacy && isTweetId(node.legacy.id_str) && node.legacy.full_text !== undefined) cur = node.legacy.id_str;
      } catch (e) {}
      try {
        const urls = variantsOf(node);
        if (urls.length && cur) {
          let set = groups.get(cur);
          if (!set) { set = new Set(); groups.set(cur, set); }
          urls.forEach(u => set.add(u));
        }
      } catch (e) {}
      for (const k in node) {
        if (k === '__proto__' || k === 'constructor' || k === 'prototype') continue;
        try { walk(node[k], cur, depth + 1); } catch (e) {}
      }
    };
    try { walk(root, null, 0); } catch (e) { return []; }
    const out = [];
    groups.forEach((set, tweetId) => {
      if (set.size) out.push({ tweetId: String(tweetId), urls: Array.from(set) });
    });
    return out;
  }
  ...
  // ── 7. Native download clicks (<a download>, KVS "Download" buttons) ─────
  // The browser's own download path never passes through our fetch/XHR hooks,
  // so capture the click here, in the MAIN world's capture phase — before the
  // page's handlers can preventDefault()/stopImmediatePropagation() it. This
  // catches members-area file URLs that carry no .mp4 extension (KVS
  // /get_file/<hash>, /dwnl/) plus the intended filename from `download=`.
  const DL_LINK_RE = /\/get_file\/|\/dwnl\/|\.(mp4|m4v|webm|mkv|mov|zip|rar|7z|ts|m3u8)(\?|#|$)/i;
  document.addEventListener('click', (e) => {
    try {
      const t = e.target;
      const a = t && t.closest ? t.closest('a[href], area[href]') : null;
      if (!a || !a.href || !/^https?:/i.test(a.href)) return;
      const isDownloadAttr = a.hasAttribute('download');
      if (!isDownloadAttr && !DL_LINK_RE.test(a.href)) return;
      postToContentScript('download-link', {
        url: a.href,
        filename: (a.getAttribute('download') || '').trim() || undefined,
      });
    } catch (err) { /* swallow */ }
  }, true);

  // ── 8. Auto-scan on media events ──────────────────────────────────────────
