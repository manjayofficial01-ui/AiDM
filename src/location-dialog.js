...
/**
 * Latest known payload per download id. Probes refine the name/size AFTER the
 * dialog was requested (IDM-style immediate open), so the queued copy would
 * be stale — drain() always shows the merged latest, and patch() merges here
 * too so a patch that lands before the window finishes loading is still
 * reflected in the initial `location-init` data instead of being lost.
 */
const latest = new Map(); // id -> payload

function requestLocation(payload) {
  if (!payload || !payload.id) return;
  latest.set(payload.id, { ...(latest.get(payload.id) || {}), ...payload });
 ...
 * it (e.g. the file name/size learned from the server after the dialog was
 * requested). Only sent while no field has been edited by the user in the
 * renderer, so it never clobbers manual input.
 */
function patch(id, patch_) {
  if (!id || !patch_) return;
  latest.set(id, { ...(latest.get(id) || {}), ...patch_ });
